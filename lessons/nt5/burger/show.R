...
readResults=function(fileName='output.dat') {
	...
	# read results
	y=array(as.matrix(read.table(fileName,skip=2,nrows=nexp*(nt+1))),c(nexp,nt+1,nx))
...
plot_fcn=function(it,expNames,L) {
	...
	plot(NA,NA,xlim=range(L$x),ylim=c(-4,4),xlab='x',ylab='phi')
	# plot experiments
...
animateResults=function(fileName='output.dat',expNames=NULL,delay=0.1) {
