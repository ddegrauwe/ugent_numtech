...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \emph{[personal assessment!]} This could make physics-based models less relevant, especially for operational weather forecasting\\[3ex]
		\item Nevertheless,\\[1ex]
		    \begin{itemize}
                \item Whether extreme events are forecasted well by an AI model is an open question\\[1ex]
...
\begin{frame}
	...
	\begin{center}
		...
		\begin{tabular}{l|lll}
			date	&	16h00	&	17h30 \\
		\hline
			21/09 	& Introduction,             &	(optional) Practicum	\\[-4pt]
					& Stability	                &   Python basics           \\
			28/09	& Time discretization		&   Practicum               \\
			05/10	& Space discretization		&		                    \\
			12/10	& (no lesson) \\
			19/10	& Spectral methods       	&	Practicum				\\
			26/10	& Nonlinearity      		&	Practicum				\\
			02/11	& (no lesson) \\
			09/11	& Semi-Implicit and		 	&	Project assignment		\\[-4pt]
					& Semi-Lagrangian models	&	       \\
		\hline
			16/11	& (no lesson) \\
			23/11	& Guest lecture on AI \\
			30/11	& Parallel computing &	Project support session \\
			07/12	& Project support session \\
			08/12	& Guest lecture on a & (morning)\\[-4pt]
                    & real weather model \\
			14/12	& Student project presentations
		\end{tabular}
		\def\arraystretch{1}
	\end{center}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Practical sessions\vspace*{1ex}
			\begin{itemize}
        		\item Software needed: python (numpy, ipympl, matplotlib), Jupyter notebooks\vspace*{2ex}
				...
				\item We will use High-Performance Computing (HPC) infrastructure of UGent: you should already have an account (?)\vspace*{1ex}
				\item access through browser via \texttt{https://login.hpc.ugent.be}\vspace*{1ex}
				\item or you can just install Jupyter on your laptop\vspace*{2ex}
			\end{itemize}
