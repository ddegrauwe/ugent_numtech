...
\begin{frame}
	...
	\begin{myitemize}{2ex}
		...
		\item So there's a limit to the speed-up that can be achieved by parallelizing! We call this the \emph{scalability} of a program.
			%
	\pause
		\item Amount of parallelism typically isn't the bottleneck in running NWP/Climate models on high-performance systems.
			%
	\end{myitemize}
	%
\end{frame}
...
\begin{frame}
	...
	\begin{myitemize}{2ex}
		...
		\item There's always a tradeoff between accuracy and computational cost
			%
		\item Power consumption is also a major factor
			%
